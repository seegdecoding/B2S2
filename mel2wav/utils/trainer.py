...
class Trainer:
    def __init__(
        ...
        self.on_device = self.num_gpus > 0
        self.root_gpu = 0
        ...
        self.move_to_device_fn = move_to_cuda
    ...
    def run_evaluation(self, test=False):
        eval_results = self.evaluate(self.task, test, tqdm_desc='Valid' if not test else 'test',
                                     max_batches=hparams.get('val_max_batches', -1) if not test
                                     else hparams.get('test_max_batches', -1))
    ...
    def evaluate(self, task, test=False, tqdm_desc='Valid', max_batches=None):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if self.on_device:
                batch = self.move_to_device_fn(batch, self.root_gpu)
    ...
    def run_training_batch(self, batch_idx, batch):
        ...
        for opt_idx, optimizer in enumerate(self.optimizers):
            ...
            with autocast(enabled=self.amp):
                if self.on_device:
                    batch = self.move_to_device_fn(copy.copy(batch), self.root_gpu)
